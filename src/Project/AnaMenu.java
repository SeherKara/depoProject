...
public class AnaMenu implements AnaMenuInterFace {
    ...
    public   void urunTanimlama() {
        Project.Urun urun=new Project.Urun();
...
//        System.out.println("Ürün başarıyla tanımlandı: " + urun);
        ...
        for (Project.Urun varOlanUrun : Project.Urun.urunList.values()) {
        ...
        if (urunVarMi) {
            ...
            int yeniId = Project.Urun.urunList.isEmpty() ? 1000 : Project.Urun.urunList.keySet().stream().max(Integer::compareTo).orElse(1000) + 1;
            urun.setId(yeniId);
            Project.Urun.urunList.put(yeniId, urun);
            System.out.println("Ürün başarıyla tanımlandı: " + urun);
        }
        girisEkrani();
    }
    ...
    public void urunListeleme() {
        ...
        System.out.println(CB+"id\t\tismi\turetcisi\t\tmiktarı\t\tbirimi\t\traf" +
                "\n-------------------------------------------------------------"+W);
        Project.Urun yeniUrun=new Project.Urun();
        for (Map.Entry<Integer, Project.Urun> entry : Project.Urun.urunList.entrySet()) {
    ...
    public void urunGirisi() {
        ...
        int Id=intGirisi();
        if (Project.Urun.urunList.keySet().contains(Id)) {
            System.out.println("Kaç tane ekleme yapmak istiyorsunuz");
            int yeniMiktar=intGirisi();
            Project.Urun urun = Project.Urun.urunList.get(Id); // Ilgili urunu alıyorum
    ...
    public  void urunuRafaKoy() {
        ...
        int id=intGirisi();
        if (Project.Urun.urunList.keySet().contains(id)) {
            System.out.println("Hangi rafa ekleme yapmak istiyorsunuz: " + Project.Urun.urunList.get(id));
            String raf=stringGirisi();
            Project.Urun.urunList.get(id).setRaf(raf);
    ...
    public  void urunCikisi() {
        ...
        if (Project.Urun.urunList.keySet().contains(id)) {
            ...
            if (cikarilanMiktar<0){
                ...
                Project.Urun urun = Project.Urun.urunList.get(id);
